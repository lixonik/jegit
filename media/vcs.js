(function () {
  ...
  function fileRow(f, depth, showDir) {
    ...
    row.addEventListener('contextmenu', (e) => {
      ...
      menu.push({ label: 'Show History', cmd: () => vscode.postMessage({ type: 'fileHistory', path: f.path }) });
      if (!f.untracked && !f.deleted) {
        menu.push({ label: 'Annotate', cmd: () => vscode.postMessage({ type: 'annotate', path: f.path }) });
      }
  ...
  document.addEventListener('keydown', (e) => {
    ...
    else if (e.key === 'PageDown') next = idx < 0 ? 0 : Math.min(idx + 10, last);
    else if (e.key === 'PageUp') next = Math.max((idx < 0 ? 0 : idx) - 10, 0);
  ...
  // ---- Log: graph layout ----
  // computeGraph lives in media/graph.js (shared with the unit tests); it is
  // exposed as the JeGitGraph global and invoked where logData arrives.
  ...
  function logRow(c, gr, withGraph) {
    ...
    row.addEventListener('contextmenu', (e) => {
      ...
      menu.push(
        ...
        { label: 'Copy Full Message', cmd: () => vscode.postMessage({ type: 'copyMessage', hash: c.hash }) },
    ...
    for (const f of node.files.slice().sort((a, b) => a.path.localeCompare(b.path))) {
      ...
      row.addEventListener('contextmenu', (e) => {
        ...
        showCtx(e, [
          ...
          { label: 'Filter Log by This Path', cmd: () => vscode.postMessage({ type: 'setLogPath', path: f.path }) },
  ...
  window.addEventListener('message', (ev) => {
    ...
    if (m.type === 'state') {
      ...
      detailsCache = {};
      const g = JeGitGraph.computeGraph(logCommits);
      graphRows = g.rows;
      maxLanes = g.maxLanes;
      populateUsers();
      renderLog();
